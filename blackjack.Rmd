...
calc_probability_exact_dealer_score <- function(dealer_up_card, target_dealer_value, num_decks, cache){
  ...
  if(target_dealer_value < 17){
    ...
    valid_one_path_permuts = Filter(function(path){
      ...
      for(card in path){
        if(card == 1 && total + 11 <= 21){
          # TODO: Forse l'asso può valere 1 se sballa con carte successive
          return(FALSE)
        }
    ...
    print(cat("Given up card ", dealer_up_card, " to reach ", target_dealer_value, " we have the following not-1 paths: "))
    print(not_1_paths)
    print(cat("Instead, given up card ", dealer_up_card, " to reach ", target_dealer_value, " we have the following 1 paths: "))
    print(valid_one_path_permuts)
...
# TODO: Rivedi
calc_probability_lose_after_standing3 <- function(player_score, dealer_up_card, num_decks, cache){
  prob = 0
  
  for(i in (player_score + 1):21){
    if(i >= 17){
      cache_key = paste(player_score, dealer_up_card, i, num_decks, sep = "_")
      if(exists(cache_key, envir = cache)){
        return(get(cache_key, envir = cache))
      }
      
      paths = find_combinations(1:11, i - dealer_up_card)
      ...
      flattened_permutations = unlist(permutations, recursive = FALSE)
      
      valid_flattened_permutations = Filter(function(path){
        is_blackjack = FALSE
    
        if(dealer_up_card == 10 && any(path[1] %in% c(1, 11))){
          is_blackjack = TRUE
        }
        if(dealer_up_card %in% c(1, 11) && path[1] == 10){
          is_blackjack = TRUE
        }
        ...
        ones_occurrences = sum(path %in% c(1))
        
        if(ones_occurrences > 0){
          total = dealer_up_card
          unmatched_ones = 0
          for(card in path){
            if(card == 1){
              unmatched_ones = unmatched_ones + 1
              total = total + 11
            }
            else{
              total = total + card
      ...
      # print(cat("For target ", i))
      # print(valid_flattened_permutations)
      
      # Calculate the probability for each valid permutation, and sum them
      probability_permutations = calculate_paths_probability(valid_flattened_permutations)
...
```{r}
calc_probability_lose_after_standing4 <- function(player_score, dealer_up_card, num_decks, cache){
  inner = function(current_score, unmatched_ones, depth){
    if(current_score > 21){
      if(unmatched_ones > 0){
        unmatched_ones = unmatched_ones - 1
    ...
    if(current_score > max(16, player_score) && current_score <= 21){
      if(unmatched_ones <= 0){
        return(1)
      }
      else{
    ...
    if(depth <= 0){
      if((current_score > max(16, player_score) && current_score <= 21) && unmatched_ones <= 0){
        return(1)
      }
      else{
        return(0)
    ...
    for(card in 1:11){
      ...
      if(dealer_up_card == 10 && current_score == dealer_up_card && any(card %in% c(1, 11))){
        is_blackjack = TRUE
      }
      if(dealer_up_card %in% c(1, 11) && current_score == dealer_up_card && card == 10){
        is_blackjack = TRUE
      }
      
      if(!is_blackjack){
        card_prob = if(card == 10) {16/52} else {4/52}
        is_one = if(card == 1) 1 else 0
        
        new_score = current_score + card
        
        if(is_one > 0){
          new_score = current_score + 11
        }
        
        inner_prob = inner(new_score, unmatched_ones + is_one, depth - 1)
        
        if(inner_prob > 0){
          # print(cat(depth, "__", card))
        }
          
        prob = prob + card_prob * inner_prob
      }
    }
  ...
  return(inner(dealer_up_card, 0, 3))
}
...
decision_matrix = sapply(possible_dealer_face_up_card, function(dealer_up_card){
  return(sapply(possible_player_scores, function(player_score){
    prob_lose_after_standing = calc_probability_lose_after_standing2(player_score, dealer_up_card, num_decks, cache)
