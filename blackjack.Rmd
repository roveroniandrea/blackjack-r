...
possible_player_scores = 13:21 # 15:21 works on single core

possible_dealer_face_up_card = 7:11 # 7:11 works on single core

num_decks = 1
soft_stands = FALSE
```

Note: **Since the following code chunks are heavy to compute, they are set with `eval = FALSE` so to not be evaluated when Knitting the rmarkdown**. Additionally, they're set with `cache = TRUE` so to not be re-evaluated at any Knit (if eval enabled)
